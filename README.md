...
- **Corretto un bias linguistico serio**: le risposte sbagliate usavano troppo spesso parole come "sempre", "mai", "solo", "esclusivamente", "unicamente" — un classico trucco da test che permette di scartare un'opzione senza saperne il contenuto. Analisi effettuata su tutta la banca dati: 554 opzioni sbagliate contro appena 11 corrette contenevano queste parole, con **49 domande completamente indovinabili al 100%** (tutte e 3 le opzioni sbagliate col trigger, la corretta senza). Tutte e 49 sono state riscritte con contenuto realmente alternativo (non solo linguisticamente diverso). Il problema derivava in parte da un mio precedente intervento poco riuscito per bilanciare la lunghezza delle risposte con frasi generiche dal tono svalutante ("un criterio che porta a risultati distorti" e simili): quelle frasi sono state rimosse e sostituite con elaborazioni neutre e tecniche, specifiche per ogni materia. Resta un residuo del 37% di domande con almeno una risposta contenente queste parole (in calo dal 46%), dovuto a contenuto tecnico legittimo nelle domande originali — un'euristica molto più debole (non elimina la risposta, solo un'opzione su tre).
- **31 domande di Statistica basate sulla TUA dispensa** (Pasquale Pavone / Paolo Sciattella / Veronica Distefano, testo Borra-Di Ciaccio) più altre 25 su probabilità/inferenza, per un totale di 56 domande grounded nel tuo materiale specifico. Copertura completa di entrambe le parti del corso, con gli stessi esempi numerici e dataset delle tue slide.
- **Revisione errori**: controllo automatico su tutte le domande (campi mancanti, opzioni duplicate, indice della risposta corretta fuori range, difficoltà non valida) e ricerca di domande duplicate. Sono state trovate e corrette **6 domande ripetute per errore** in due lotti diversi; sono state rimosse e sostituite con domande nuove su argomenti non ancora trattati.
- **Ulteriore ampliamento**: la banca fissa è cresciuta ancora, per un totale di **554 domande** (partite da 220).
...
|---|---|---|
| 📊 Statistica | 176 (51 facili · 78 medie · 47 difficili, incluse 56 basate sulla tua dispensa) | ✅ calcolo (15 modelli) |
| 🏢 Economia Aziendale | 88 (31 facili · 33 medie · 24 difficili) | ✅ definizioni (56 termini) |
...
**Totale: 554 domande nella banca fissa** (numero verificato via conteggio automatico — una versione precedente di questo file riportava 585 per un mio errore di calcolo), più gli esercizi infiniti generati al volo per ogni materia.
